...
parser.add_argument('--mode',
                    choices=['finetune', 'prune', 'inference', 'validation'],
                    help='Run mode')
...
parser.add_argument('--class_name', type=str, help='')
...
def main():
    ...
    if 'cropped' in args.dataset:
        ...
        manager = Manager(args, model, shared_layer_info, masks, train_loader=None, val_loader=None, begin_prune_step=None, end_prune_step=None)
        test_loader = dataset.test_loader(args.test_path, args.val_batch_size)
        manager.load_checkpoint_only_for_evaluate(resume_from_epoch, resume_folder)
        manager.validate(test_loader, resume_from_epoch-1, args.class_name)
        return 
    else:
    ...
    if args.mode == 'inference':
        predict_loader = dataset.predict_loader(args.pred_path, args.val_batch_size)
        manager.load_checkpoint_only_for_evaluate(resume_from_epoch, resume_folder)
        manager.predict(predict_loader, resume_from_epoch-1, args.dataset)
        return
